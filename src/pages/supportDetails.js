...
const SupportDetails = (props) => {
    ...
    const  { data: ratesList } =  useSWR("/projects/" + props.match.params.support_id + "/rates.json")
    ...
    function App () {
        var { data, size, setSize, isValidating, error } = useSWRInfinite(getKey, Fetcher)
...
const Comments =  props => {
    ...
    const handleSubmit = (event) => {
        ...
        if(textarea){
            ...
            message = "<div class='username'>"+props.user.username+"</div><div class='email'>"+props.user.attributes.email+"</div><div class='sub'>"+props.user.attributes.sub+"</div></div><br />"+draftToHtml(convertToRaw(textarea.getCurrentContent()))
            ...
            // fetch('http://127.0.0.1:3003/general/notification', {   
            //     method: 'POST',
            ...
            <div className="row">
                <table className="tablesorter eael-data-table center">
                    ...
                    <tbody>
                        ...
                        <td>{projectItem['estimated-minutes'] && timeConvert(projectItem['estimated-minutes'])}</td>
                        <td>{billingRates && Formatter(Math.round(timeConvert(projectItem['estimated-minutes']) * billingRates.rate))} </td>
                        <td  className='status'  style={{textAlign:"right"}}> {projectItem['boardColumn'] && <> <i class="fa fa-circle" style={{color:projectItem['boardColumn'].color}}></i>  {projectItem['boardColumn'].name}</>}</td>
                                ...
                                {projectItem.description &&
                                    <div className="row previewSimple">
                                       <SimpleMDE
